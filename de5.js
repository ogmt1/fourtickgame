...
var player2Color = 'rgb(237, 45, 233)';
...
$('.board button').on('click', function(){
    ...
    if(horizontalWinCheck() || verticalWinCheck() || diagonalWinCheck()){
        $('h1').tex(currentName+ "You have won the Game! ")
        $('h3').fadeOut('fast');
        $('h3').fadeOut('fast');
    }
    ...
    for (var col = 0; col < 4; col++) {
        for (var row = 0; row < 6; row++) {
            if (colorMatchCheck(returnColor(row, col), returnColor(row + 1, col + 1), returnColor(row + 2, col + 2), returnColor(row + 3, col + 3))) {
                ...
                reportWin(row, col);
                return true;
            } else if (row >= 3 && colorMatchCheck(returnColor(row, col), returnColor(row - 1, col + 1), returnColor(row - 2, col + 2), returnColor(row - 3, col + 3))) {
                console.log('diag');
                reportWin(row, col);
                return true;
            } else {
